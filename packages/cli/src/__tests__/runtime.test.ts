...
describe("createBrokerRuntime", () => {
  ...
  test("start() initializes services in dependency order", async () => {
    ...
    // Key manager initialized first, then local core init, then ws/admin
    // servers. Full core init is deferred until a harness request needs it.
    ...
    expect(wsIdx).toBeGreaterThan(localIdx);
  ...
  test("startup does not eagerly initialize the full broker core", async () => {
    const tracker = createCallTracker();
    const config = makeConfig(tempDir);
    const deps = makeMockDeps(tracker);

    deps.createBrokerCore = () => {
      tracker.record("brokerCore.create");
      return {
